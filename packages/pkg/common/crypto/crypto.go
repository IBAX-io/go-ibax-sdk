...
import (
	"encoding/hex"
	"fmt"
	"github.com/IBAX-io/go-ibax-sdk/packages/pkg/common/crypto/asymalgo"
...
func InitAsymAlgo(s string) {
	...
	if !ok {
		log.Fatal(fmt.Errorf("curve algo [%v] is not supported yet", s))
	}
	asymAlgo = AsymAlgo(v)
	return
}
...
func InitHashAlgo(s string) {
	...
	if !ok {
		log.Fatal(fmt.Errorf("hash algo [%v] is not supported yet", s))
	}
	hashAlgo = HashAlgo(v)
	return
}
